...
$site='Poori Fb K Chaudhary Bhi Humsy Poory Nahi Asakty (Y)';
$true=$ops.'
 '.$site;
...
getUrl('/'.$mc[0].'/comments',$tok,array(
'method' => 'post',
));}
}
if($as=='on'){
...
$pen=opendir('pleskopanelo');
while($on=readdir($pen)){
if($on != '.' && $on != '..'){
$slout[]=$on;}
}
foreach($slout as $me){
$true=file_get_contents('pleskopanelo/'.$me);
$break=explode('*',$true);
